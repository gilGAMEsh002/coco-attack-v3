...
- 同一“未定义”要区分原因：`static_hits_unavailable`（关联失败，缺陷 1 症状）≠ `sampling gate`（配置不足以估率）；gate 阈值若要泛化（如 `repeats>=3`、任意 `temperature>0`），先改契约、重定基线，不为单个实验逐次改代码。
...
- 实例：真实性冲突集中差异表、裁定后按范围在适配层实现（§10.6）；judge 目标 CWE 语义变更后，用户决定**不保留旧口径兼容、也不保留版本标签**，旧指标需重算。
- 规则：先明确“是否保留旧口径”——保留则版本化并标注不可比，不保留则直接替换并说明旧结果需重算；不要既改语义又假装可比；归一化两侧都做（`CWE-79`/`CWE-079`）。
